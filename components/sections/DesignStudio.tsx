    ...
    <section id="studio" className="py-24 bg-zinc-950 overflow-hidden">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <div className="mt-6 max-w-xl">
                ...
                <div className="flex gap-4 mt-4">
                  {project.features.map((f, i) => (
                    <span key={i} className="text-[10px] font-mono text-zinc-500 border border-zinc-800 px-2 py-1 uppercase">
                      {f}
                    </span>
                  ))}
                </div>
          ...
          {/* Nueva sección: Gimnasios */}
          <motion.article
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className="relative col-span-12 lg:col-span-6 group"
          >
            <div className="relative aspect-[16/9] overflow-hidden rounded-sm border border-zinc-800">
              <img
                src="demogym-20metors-nuevo"
                alt="Gimnasios en contenedor"
                className="w-full h-full object-cover grayscale brightness-75 group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-1000"
                loading="lazy"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60 group-hover:opacity-40 transition-opacity"></div>
              ...
              <div className="absolute bottom-10 left-10 right-10">
                <span className="font-mono text-orange-500 text-xs mb-2 block uppercase tracking-widest">Gimnasios</span>
                <div className="flex justify-between items-end">
                  <h3 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Desde 9900€</h3>
                  <div className="p-4 bg-orange-600 rounded-full opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                    <ArrowUpRight size={24} className="text-white" />
                  </div>
                </div>
            ...
            <div className="mt-6 max-w-xl">
              <p className="text-zinc-400 leading-relaxed font-light">
                Contenedor de 20 pies con ventanal de aluminio de apertura frontal, ventilación propia y un interior muy luminoso para entrenamiento funcional.
              </p>
              <div className="flex flex-wrap gap-4 mt-4">
                <span className="text-[10px] font-mono text-zinc-500 border border-zinc-800 px-2 py-1 uppercase">20 pies</span>
                <span className="text-[10px] font-mono text-zinc-500 border border-zinc-800 px-2 py-1 uppercase">Ventanal frontal</span>
                <span className="text-[10px] font-mono text-zinc-500 border border-zinc-800 px-2 py-1 uppercase">Ventilación propia</span>
                <span className="text-[10px] font-mono text-zinc-500 border border-zinc-800 px-2 py-1 uppercase">Muy luminoso</span>
              </div>
            </div>
          </motion.article>
        </div>
